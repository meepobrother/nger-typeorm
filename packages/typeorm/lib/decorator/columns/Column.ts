...
} from "../../driver/driver-types/ColumnTypes";
...
type _ColumnTypeOrOptions = ((type?: any) => Function) | ColumnType | (ColumnOptions & ColumnEmbeddedOptions)
export type _ColumnOptions = [
    _ColumnTypeOrOptions,
...
export function Column(...args: any[]) {
    return createPropertyDecorator<_ColumnOptions>(`Column`, it => {
        // normalize parameters
        let [typeOrOptions, options] = it.options as _ColumnOptions;
        ...
        //  Reflect && (Reflect as any).getMetadata ? (Reflect as any).getMetadata("design:type", it.type, it.property) : undefined;
        ...
        if (typeOrOptions instanceof Function) { // register an embedded
            ...
            if (options.unique === true)
                getMetadataArgsStorage().uniques.push({ target: it.type, columns: [it.property as string] });
